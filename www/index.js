import { handleMIDIMessage } from "./midi.js";
import {
  updateSwitch,
  updateKnob,
  updateLeds,
...
async function initialize() {
  ...
  let input_devices_element = document.querySelector(
    "#midi_input_devices_container"
  );
  let output_devices_element = document.querySelector(
    "#midi_output_devices_container"
  );

  try {
    const midi_access = await navigator.requestMIDIAccess({
      sysex: true,
      software: false,
    });
    ...
    for (const [id, device] of midi_access.inputs) {
      let midi_device_element = midi_item_element(
        template,
        device,
        (device) => {
          ...
          render_selected_device_element(
            "#midi_input_device_selected_item",
            device.name
          );
        }
      );
    ...
    for (const [id, device] of midi_access.outputs) {
      let midi_device_element = midi_item_element(
        ...
        (device) => {
          ...
          render_selected_device_element(
            "#midi_output_device_selected_item",
            device.name
          );
        }
      );
...
function render_save_patch_element(querySelector, program) {
  ...
  let data =
    ...
    encodeURIComponent(JSON.stringify(program, undefined, 2));
...
function render_retrieve_patch(querySelector, midiDevice, onClick) {
  ...
  let retrieve_patch_status = retrieve_patch_btn
    .querySelector(".connection_status")
    .classList.replace(
...
function midi_item_element(template, midiDevice, onClick) {
  ...
  component.querySelector("#midi_device_item__name").innerHTML =
    midiDevice.name;
...
function updateFXSection(program) {
  updateSwitch(
    "#effect_mod .switch",
    program.mod_effect_enabled,
    (w) => false,
    (w) => w == false,
    (w) => w == true
  );
  updateSwitch(
    "#effect_reverbdelay .switch",
    program.delay_reverb_effect_enabled,
    (w) => false,
    (w) => w == false,
    (w) => w == true
  );
  ...
  updateSwitch(
    "#vcf-lowcut .switch",
    program.timbre1.low_cut_enabled,
    (w) => w,
    (w) => !w,
    (w) => false
  );
  ...
  updateKnob(
    "#effect_speed .knob",
    "#effect_speed .knob-value-label",
    program.mod_effect_speed
  );
  updateKnob(
    "#effect_depth .knob",
    "#effect_depth .knob-value-label",
    program.mod_effect_depth
  );
  updateKnob(
    "#effect_reverbdelay_time .knob",
    "#effect_reverbdelay_time .knob-value-label",
    program.delay_reverb_time
  );
  updateKnob(
    "#effect_reverbdelay_depth .knob",
    "#effect_reverbdelay_depth .knob-value-label",
    program.delay_reverb_depth
  );

  // Update mod effects name
  if (program.mod_effect_enabled) {
    document.querySelector("#mod_effect_type").innerHTML =
      program.mod_effect_type;
  } else {
    document.querySelector("#mod_effect_type").innerHTML = "";
  }
...
function updateFilterSection(program) {
  updateKnob(
    "#vcf-cutoff .knob",
    "#vcf-cutoff .knob-value-label",
    program.timbre1.cutoff
  );
  updateKnob(
    "#vcf-resonance .knob",
    "#vcf-resonance .knob-value-label",
    program.timbre1.resonance
  );
  updateGenericKnob(
    "#vcf-egint .knob",
    "#vcf-egint .knob-value-label",
    program.timbre1.cutoff_eg_int,
    false,
    -4800,
    4800
  );
  ...
  updateSwitch(
    "#vcf-keytrack .switch",
    program.timbre1.cutoff_keyboard_track,
    (w) => w == "High",
    (w) => w == "Low",
    (w) => w == "Mid"
  );
...
function updateMixerSection(program) {
  updateKnob(
    "#mixer-vco1-volume .knob",
    "#mixer-vco1-volume .knob-value-label",
    program.timbre1.vco1_level
  );
  updateKnob(
    "#mixer-vco2-volume .knob",
    "#mixer-vco2-volume .knob-value-label",
    program.timbre1.vco2_level
  );
  updateKnob(
    "#mixer-multi-volume .knob",
    "#mixer-multi-volume .knob-value-label",
    program.timbre1.multi_level
  );
}

function updateOscillatorsSection(program) {
  updateLeds(
    "#vco1-octave .switch-labels",
    program.timbre1.vco_1_octave,
    (d) => d == 3,
    (d) => d == 2,
    (d) => d == 1,
    (d) => d == 0
  ...
  updateLeds(
    "#vco2-octave .switch-labels",
    program.timbre1.vco_2_octave,
    (d) => d == 3,
    (d) => d == 2,
    (d) => d == 1,
    (d) => d == 0
  ...
  updateLeds(
    "#multi-octave .switch-labels",
    program.timbre1.multi_octave,
    (d) => d == 3,
    (d) => d == 2,
    (d) => d == 1,
    (d) => d == 0
  ...
  updateKnob(
    "#vco1-shape .knob",
    "#vco1-shape .knob-value-label",
    program.timbre1.vco_1_shape
  );
  updateKnob(
    "#vco2-shape .knob",
    "#vco2-shape .knob-value-label",
    program.timbre1.vco_2_shape
  );
  updateKnob(
    "#multi-shape .knob",
    "#multi-shape .knob-value-label",
    program.timbre1.shape_noise
  );

  updateGenericKnob(
    "#vco1-int .knob",
    "#vco1-int .knob-value-label",
    program.timbre1.pitch_eg_int,
    false,
    -4800,
    4800
  );
  updateKnob(
    ...
    "#cross-mod-depth .knob-value-label",
    program.timbre1.cross_mod_depth
  );

  updateKnob(
    "#vco1-pitch .knob",
    "#vco1-pitch .knob-value-label",
    program.timbre1.vco_1_pitch
  );
  updateKnob(
    "#vco2-pitch .knob",
    "#vco2-pitch .knob-value-label",
    program.timbre1.vco_2_pitch
  );

  updateSwitch(
    "#vco1-wave .switch",
    program.timbre1.vco_1_wave,
    (w) => w == "Saw",
    (w) => w == "Square",
    (w) => w == "Triangle"
  );
  ...
  updateSwitch(
    "#vco1-pitch-eg .switch",
    program.timbre1.pitch_eg_target,
    (w) => w == "Plus",
    (w) => w == "Vco2",
    (w) => w == "Vco1"
  );
...
function updateEnveloppeSection(program) {
  updateKnob(
    "#lfo-rate .knob",
    "#lfo-rate .knob-value-label",
    program.timbre1.lfo_rate
  );
  updateKnob(
    "#lfo-int .knob",
    "#lfo-int .knob-value-label",
    program.timbre1.lfo_int
  );

  updateKnob(
    "#amp-attack .knob",
    "#amp-attack .knob-value-label",
    program.timbre1.amp_eg_attack
  );
  updateKnob(
    "#amp-decay .knob",
    "#amp-decay .knob-value-label",
    program.timbre1.amp_eg_decay
  );
  updateKnob(
    "#amp-sustain .knob",
    "#amp-sustain .knob-value-label",
    program.timbre1.amp_eg_sustain
  );
  updateKnob(
    "#amp-release .knob",
    "#amp-release .knob-value-label",
    program.timbre1.amp_eg_release
  );

  updateKnob(
    "#filter-eg-attack .knob",
    "#filter-eg-attack .knob-value-label",
    program.timbre1.eg_attack
  );
  updateKnob(
    "#filter-eg-decay .knob",
    "#filter-eg-decay .knob-value-label",
    program.timbre1.eg_decay
  );
  updateKnob(
    "#filter-eg-sustain .knob",
    "#filter-eg-sustain .knob-value-label",
    program.timbre1.eg_sustain
  );
  updateKnob(
    "#filter-eg-release .knob",
    "#filter-eg-release .knob-value-label",
    program.timbre1.eg_release
  );

  updateSwitch(
    "#lfo-wave .switch",
    program.timbre1.lfo_wave,
    (w) => w == "Saw",
    (w) => w == "Square",
    (w) => w == "Triangle"
  );
...
function updateTimbreSection(program) {
  updateKnob(
    "#timbre-mainsub-balance .knob",
    "#timbre-mainsub-balance .knob-value-label",
    program.main_sub_balance
  );
  updateSwitch(
    "#timbretype .switch",
    program.timbre_type,
...
function updateVoiceSection(program) {
  updateKnob(
    "#portamento .knob",
    "#portamento .knob-value-label",
    program.timbre1.portamento_time
  );
  updateKnob(
    "#voice-spread .knob",
    "#voice-spread .knob-value-label",
    program.timbre1.voice_spread
  );
  updateKnob(
    "#voice-mode-depth .knob",
    "#voice-mode-depth .knob-value-label",
    program.timbre1.voice_mode_depth
  );
}
...
document
  .querySelector(".full-screen-menu__close")
...
load_patch_link_el.addEventListener(
  "click",
  (event) => {
